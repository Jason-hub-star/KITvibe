...
export function AISpotlightCard({ summaries, totalQuestions }: Props) {
  ...
  return (
    <div className="bg-surface-high border border-border text-foreground p-8 flex justify-between items-start gap-8 overflow-hidden relative">
      <div className="flex flex-col gap-4 z-10">
        ...
        <a
          href="#lesson-report"
          className="border border-border bg-background px-6 py-2 text-xs font-bold tracking-widest uppercase w-fit transition-colors hover:bg-card"
        >
          리포트 확인
        </a>
      </div>
      <span className="text-[120px] font-black leading-none text-foreground/10 select-none absolute right-8 top-1/2 -translate-y-1/2">
        AI
      </span>
    </div>
  );
