  ...
  return (
    <div className="space-y-4">
      <div className="grid md:grid-cols-2 gap-px bg-border border border-border">
        {ROLES.map(({ role, label, sublabel, description, progressStart, progressHover }, index) => (
          ...
          >
            ...
            <p className="ui-support text-muted-foreground mb-12 whitespace-pre-line">
              {description}
            </p>
            {/* Progress Bar */}
            ...
            {loading === role && (
              <span className="ui-micro text-muted-foreground animate-pulse mt-4 block">
                준비 중...
              </span>
            )}
          </button>
      ...
      <p className="ui-micro text-muted-foreground">
        데모용 역할 선택입니다. 실제 운영 단계에서는 서버 권한 검증과 RLS 기준으로 접근을 분리합니다.
      </p>
    </div>
  );
}
