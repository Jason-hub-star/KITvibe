...
export default function ModeSelector({ mode, onModeChange, disabled = false }: ModeSelectorProps) {
  return (
    <div className="flex gap-2 md:gap-4 items-center border-b border-border">
      {MODES.map(({ key, label, desc }) => (
        <button
          key={key}
          onClick={() => onModeChange(key)}
          disabled={disabled}
          className={`flex-1 min-w-0 transition-colors pb-2 md:pb-3 -mb-px flex flex-col items-start ${
            mode === key
