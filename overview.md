...
9. 데모 역할 선택은 탐색 UX일 뿐, 실제 보안 경계는 서버 권한 검증과 RLS에 둔다
...
- **로그인 없이 탐색 가능한 데모 진입 구조** (teacher/student 버튼으로 역할 분기, 보안 경계 아님)
...
| 로그인/접속 마찰 | 🟡 높음 | 데모는 역할 선택으로 진입 마찰을 줄이고, 운영 보안은 authenticated 세션 + RLS로 분리 |
...
| 항목 | MVP 대응 | v2 대응 |
|------|---------|---------|
| 수집 최소화 | 이름 + 역할만 (데모 세션, 최소 데이터) | 학교 계정 연동 시 최소 필드 |
| 미성년 데이터 | 보호자 동의 프로세스 문서화 | 학교 단위 동의 체계 |
| 국외 이전 | Gemma 4 로컬 옵션 제공 | 국내 클라우드 (네이버/카카오) 옵션 |
| 보관 기간 | 학기 종료 시 자동 삭제 | 교사가 보관 기간 설정 |
| 접근 권한 | 역할 선택 기반 UI 가드 + 서버 write 경로 분리 (데모용, 보안 경계 아님) | authenticated + RLS + teacher/student ownership 검증 |

### M-3-1. 데모 보안 설명 원칙

- 현재 `teacher/student` 역할 선택은 심사위원이 빠르게 흐름을 탐색하기 위한 UX이다.
- 실제 보안 경계는 `서버 권한 검증`, `RLS`, `소유권 확인`으로 설명한다.
- 따라서 데모 편의와 운영 권한 모델은 같은 것으로 표현하지 않는다.

### M-4. 실행 권고 (9개 페르소나 분석 기반)
...
9. **데모 UX와 운영 보안 분리 설명**: 역할 선택은 빠른 탐색용, 실제 권한 모델은 authenticated + RLS로 확장
