...
실주행 QA는 비용과 시간을 고려해 "최소 비용 QA 매트릭스" 기준으로 운영한다.

- 항상 실행: 업로드, 학생 질문, 대시보드, Full Run
- 조건부 실행: 모드 전환, 모바일, 에러 처리, 링크/빈 상태
- 병렬 실행은 최대 3개까지로 제한
- 배포 직전 게이트는 `Full Run 1회 + 모바일 스모크 1회`

상세 기준: `docs/qa/QA-TEST-SCENARIOS.md`

## 2026-04-09 대용량 PDF 업로드 경로 수정 예정

Production에서 `multipart -> Next API` 경로가 약 4.3MB부터 `413 FUNCTION_PAYLOAD_TOO_LARGE`에 걸리는 것을 확인했다.

- 코드상 제한은 10MB였지만, 실배포 한계는 플랫폼 요청 바디 제한에 먼저 막혔다
- 대응 방향은 `signed upload URL 발급 -> 브라우저에서 Supabase Storage direct upload -> 서버 후처리`로 잠금
- 목표는 SSOT 10MB 한도를 유지하면서도 Vercel 요청 바디 한계를 우회하는 것이다

## 2026-04-09 UI 보강 패치 잠금

대시보드 CTA, 전체 primary 톤, 학생/교사 수업 검색, 채팅 입력 구조, 4단계 의미 노출을 한 번에 수정하기 전에 패치 범위를 먼저 잠갔다.

- `리포트 확인`은 별도 페이지가 아니라 현재 대시보드 내 상세 영역 이동으로 잠금
- 오개념 히트맵은 신규 기능이 아니라 기존 기능 노출 개선으로 정의
- 전체 `--primary`는 순수 검정 대신 graphite 계열로 완화 예정
- 학생/교사 수업 선택은 공통 검색 UX로 정리 예정
- 단계 로직은 유지하고, 단계 의미/이동 조건/보상 노출을 강화 예정

상세 문서: `docs/status/UI-REMEDIATION-LOCK-2026-04-09.md`

## 2026-04-09 UI 보강 패치 반영

잠금 문서 기준으로 1차 UI 보강을 반영했다.

- 교사 대시보드 상세 화면에 목록 복귀 링크 추가
- `AI 보충 분석`의 `리포트 확인` CTA를 현재 대시보드 리포트 영역 이동으로 연결
- global `--primary`를 순수 검정에서 graphite 계열로 완화
- 학생/교사 수업 선택 화면에 제목/주제/과목 검색 추가
- 학생 채팅 입력창 버튼을 입력 셸 내부 정렬 구조로 재배치
