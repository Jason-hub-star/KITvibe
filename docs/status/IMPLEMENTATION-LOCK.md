...
- 2026-04-08 실사용 완주 테스트에서 `student_questions.session_id` 미연결과 `sessions.current_step` 비누적이 확인됐다.
- 구조 잠금 자체는 유지하고, 해결은 `useQuestionChat`의 세션 race 제거와 상태 누적 로직 안정화 범위 안에서 처리한다.
- 상세 수정 순서는 `docs/status/REMEDIATION-PLAN-2026-04-08.md`를 따른다.
