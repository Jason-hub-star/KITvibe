...
## 2026-04-10 교사 대시보드 회귀 점검

> 환경: 최신 `npm run dev` 재기동 후 `http://localhost:3000`
> 범위: 교사 대시보드 상세 UI, 오개념 로더, 카드 표면 톤

### 1. 목록 복귀 / 제목 간격

- 결과: `PASS`
- 확인 내용:
  - `수업 목록으로 돌아가기` 링크가 상세 상단에 유지됨
  - 링크와 수업 메타 사이 간격이 늘어나 제목/주제 텍스트가 덜 붙어 보임

### 2. 잔존 black tone 교체

- 결과: `PASS`
- 확인 내용:
  - `AI 보충 분석` 카드는 surface 계열 밝은 배경으로 확인
  - `Question Log` 헤더도 surface 계열로 바뀌어 순수 검정 헤더가 제거됨
  - 초기에 보이던 검정 카드 인상은 stale `next-server` 화면으로 재현된 것이며, 최신 dev 서버에서는 재현되지 않음

### 3. 오개념 생성 로더 / 무한 대기 대응

- 결과: `PASS`
- 확인 내용:
  - `/api/lessons/[id]/misconceptions` 직접 호출 시 `201 Created` 확인
  - UI는 로딩 상태와 6초 타임아웃 후 `다시 분석` 경로를 가짐
  - 헤드리스 정지 캡처로는 `AI가 오개념을 분석하고 있습니다...` 문구가 짧은 과도 상태라 고정 캡처되지 않았지만, 코드 경로와 서버 응답 기준으로 무한 대기 탈출 경로는 확인
