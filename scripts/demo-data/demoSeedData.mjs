...
export const DEMO_LESSONS = [
  {
    ...
    contextCache: {
      summaryText: '인수분해는 다항식을 곱의 꼴로 바꾸는 과정이며, 데모에서는 두 수의 곱과 합 조건을 읽어 괄호를 만드는 문제에 집중한다.',
      keyConcepts: ['두 수의 곱과 합 조건', '부호 판단', '최고차항 계수가 1인 이차식 인수분해'],
      commonMistakes: ['곱 조건만 보고 합의 부호를 놓침', '상수항이 양수일 때 부호를 성급하게 결정함'],
      solutionTemplate: '1) 상수항의 약수를 찾는다. 2) 곱이 상수항, 합이 일차항 계수가 되는 두 수를 고른다. 3) 그 두 수를 괄호 안에 넣고 합의 부호를 다시 확인한다.',
    },
    materials: [
      {
...
부호를 결정할 때는 두 수의 곱과 합을 동시에 확인해야 한다.`,
    ...
    quickAnswers: [
      {
        triggerPhrase: '답만',
        questionPattern: 'x^2-5x+6 바로 풀어줘',
        conceptName: '인수분해',
        answerText: '바로 풀어볼게요. 곱해서 6이 되고 더해서 -5가 되는 두 수는 -2와 -3이에요. 그래서 x^2-5x+6=(x-2)(x-3)입니다. 최종 답은 (x-2)(x-3)예요. 실수 포인트는 두 수의 곱만 보지 말고 합의 부호도 같이 확인하는 거예요.',
      },
    ],
    summaries: [
      {
  ...
  },
  {
    ...
    contextCache: {
      summaryText: '이차방정식 단원은 근의 공식과 판별식 해석을 함께 다루며, 인수분해 가능 여부를 먼저 점검하는 습관까지 연결하는 것이 핵심이다.',
      keyConcepts: ['근의 공식', '판별식', 'a,b,c 대입 순서'],
      commonMistakes: ['b의 부호를 반대로 넣음', '판별식 값을 구하고도 해의 개수와 연결하지 못함'],
      solutionTemplate: '1) 식을 ax^2+bx+c=0 꼴로 정리한다. 2) a,b,c를 읽어 근의 공식에 대입한다. 3) 판별식을 계산한다. 4) 두 해와 해의 개수를 함께 정리한다.',
    },
    materials: [
      {
...
인수분해가 가능한 경우에는 인수분해로 먼저 접근할 수 있다.`,
    ...
    quickAnswers: [
      {
        triggerPhrase: '시간 없어',
        questionPattern: '근의 공식으로 바로 풀어줘',
        conceptName: '이차방정식',
        answerText: '바로 풀어볼게요. x^2-4x+3=0에서 a=1, b=-4, c=3이에요. 근의 공식에 대입하면 x={4±루트(16-12)}/2={4±2}/2라서 x=1 또는 x=3입니다. 최종 답은 1과 3이에요. 실수 포인트는 b값의 부호를 그대로 가져오는 거예요.',
      },
    ],
    summaries: [
      {
  ...
  },
  {
    ...
    contextCache: {
      summaryText: '일차함수 그래프 읽기 단원은 기울기와 y절편을 구분하고, 그래프의 방향과 실제 수치 계산을 연결하는 연습이 중심이다.',
      keyConcepts: ['기울기', 'y절편', '변화량 해석'],
      commonMistakes: ['기울기와 y절편을 같은 값으로 혼동함', '세로 변화량만 보고 기울기를 판단함'],
      solutionTemplate: '1) y축과 만나는 점을 찾아 y절편을 읽는다. 2) 두 점의 y 변화량과 x 변화량을 구한다. 3) 기울기 = y 변화량 / x 변화량으로 계산한다.',
    },
    materials: [
      {
...
y절편은 그래프가 y축과 만나는 점의 y값이다.`,
    ...
    quickAnswers: [
      {
        triggerPhrase: '빨리',
        questionPattern: '기울기 구하는 법 빨리 알려줘',
        conceptName: '일차함수',
        answerText: '바로 정리할게요. 기울기는 세로 변화량을 가로 변화량으로 나눈 값이에요. 두 점이 있으면 y값 차이 ÷ x값 차이를 계산하면 됩니다. 최종 답은 “기울기 = y의 변화량 / x의 변화량”이에요. 실수 포인트는 세로 변화량만 보고 기울기를 판단하는 거예요.',
      },
    ],
    summaries: [
      {
  ...
  },
  {
    ...
    contextCache: {
      summaryText: '연립방정식 활용문제는 문장 속 관계를 식으로 바꾸는 단계가 가장 중요하며, 미지수 정의와 식 세우기 순서를 먼저 고정하는 훈련이 필요하다.',
      keyConcepts: ['미지수 정의', '식 세우기', '가감법/대입법 목적'],
      commonMistakes: ['x와 y가 무엇을 의미하는지 먼저 정하지 않음', '문장 두 개를 같은 식으로 옮겨 적음'],
      solutionTemplate: '1) 문제에서 구하려는 양을 x, y로 정의한다. 2) 문장마다 관계를 하나씩 식으로 옮긴다. 3) 만든 두 식이 서로 다른 조건인지 확인한다. 4) 가감법 또는 대입법으로 푼다.',
    },
    materials: [
      {
...
식 두 개를 세운 뒤에는 단위를 확인한다.`,
    ...
    quickAnswers: [
      {
        triggerPhrase: '바로 풀어줘',
        questionPattern: '연립방정식 문장제에서 식 세우는 순서만 알려줘',
        conceptName: '연립방정식',
        answerText: '바로 풀어볼게요. 먼저 구하려는 두 값을 x와 y로 정하고, 문제 속 조건 두 개를 각각 식으로 바꿉니다. 그다음 가감법이나 대입법으로 한 문자를 없애면 됩니다. 최종 답은 “미지수 정의 → 식 두 개 세우기 → 한 문자 없애기 → 나머지 값 구하기” 순서예요. 실수 포인트는 미지수 정의를 건너뛰는 거예요.',
      },
    ],
    summaries: [
      {
  ...
  },
  {
    ...
    contextCache: {
      summaryText: '도형의 닮음 단원은 대응점 순서와 닮음비 해석이 핵심이며, 길이비와 넓이비가 어떻게 달라지는지를 분리해 생각하는 훈련이 필요하다.',
      keyConcepts: ['대응점 순서', '닮음비', '넓이비'],
      commonMistakes: ['대응변을 각 순서 없이 바로 짝지음', '길이비와 넓이비를 같은 값으로 둠'],
      solutionTemplate: '1) 대응점 순서를 먼저 같은 방향으로 적는다. 2) 대응변을 찾아 닮음비를 정한다. 3) 길이비는 그대로, 넓이비는 닮음비의 제곱으로 계산한다.',
    },
    materials: [
      {
...
대응 순서를 바꾸면 비례식이 틀어질 수 있다.`,
    ...
    quickAnswers: [
      {
        triggerPhrase: '답만',
        questionPattern: '닮음비가 2일 때 넓이비가 왜 4인지',
        conceptName: '도형의 닮음',
        answerText: '바로 설명할게요. 닮음비가 2라는 건 가로와 세로 길이가 각각 2배라는 뜻이에요. 넓이는 두 길이를 곱하므로 2×2=4가 됩니다. 최종 답은 넓이비가 4라는 거예요. 실수 포인트는 길이비와 넓이비를 같은 값으로 두는 거예요.',
      },
    ],
    summaries: [
      {
