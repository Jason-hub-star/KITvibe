...
실수 포인트:
두 수의 곱만 보지 말고 합의 부호도 같이 확인하는 거예요.`,
      },
    ],
    summaries: [
      {
    ...
    quickAnswers: [
      {
        ...
        conceptName: '이차방정식',
        answerText: `핵심 개념:
근의 공식에 a, b, c를 정확한 부호로 넣는 거예요.

풀이:
x^2-4x+3=0에서 a=1, b=-4, c=3이에요. 근의 공식에 대입하면 x={4±루트(16-12)}/2={4±2}/2라서 x=1 또는 x=3입니다.

최종 답:
x=1 또는 x=3

실수 포인트:
b값의 부호를 바꾸지 말고 그대로 가져오는 거예요.`,
      },
    ],
    summaries: [
      {
    ...
    quickAnswers: [
      {
        ...
        conceptName: '일차함수',
        answerText: `핵심 개념:
기울기는 세로 변화량을 가로 변화량으로 나눈 값이에요.

풀이:
두 점이 있으면 먼저 y값 차이와 x값 차이를 각각 구한 뒤, y값 차이 ÷ x값 차이를 계산하면 됩니다.

최종 답:
기울기 = y의 변화량 / x의 변화량

실수 포인트:
세로 변화량만 보고 기울기를 판단하면 안 돼요.`,
      },
    ],
    summaries: [
      {
    ...
    quickAnswers: [
      {
        ...
        conceptName: '연립방정식',
        answerText: `핵심 개념:
문장제를 풀 때는 미지수 정의와 식 두 개 세우기가 먼저예요.

풀이:
먼저 구하려는 두 값을 x와 y로 정하고, 문제 속 조건 두 개를 각각 식으로 바꿉니다. 그다음 가감법이나 대입법으로 한 문자를 없애면 됩니다.

최종 답:
미지수 정의 → 식 두 개 세우기 → 한 문자 없애기 → 나머지 값 구하기

실수 포인트:
미지수 정의를 건너뛰면 식의 의미가 바로 흔들려요.`,
      },
    ],
    summaries: [
      {
    ...
    quickAnswers: [
      {
        ...
        conceptName: '도형의 닮음',
        answerText: `핵심 개념:
닮음비는 길이비이고, 넓이비는 그 제곱이에요.

풀이:
닮음비가 2라는 건 가로와 세로 길이가 각각 2배라는 뜻이에요. 넓이는 두 길이를 곱하므로 2×2=4가 됩니다.

최종 답:
넓이비는 4예요.

실수 포인트:
길이비와 넓이비를 같은 값으로 두면 안 돼요.`,
      },
    ],
    summaries: [
      {
